...
Это базовая стратегия (клауза как метавершина первого уровня). Для каждой
клаузы собираются все узлы и рёбра графа, привязанные к этой клаузе
(по clause_id), и оборачиваются в GraphFragment внутри MetaNode.
Метарёбра на первом проходе не создаются — это задача следующей итерации
(structural/linguistic).

Инварианты:
- каждая клауза отражена ровно одной метавершиной уровня 1;
- ни один узел/ребро не теряется «молча»: если к клаузе не привязано
