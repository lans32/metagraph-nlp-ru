...
форма сохраняется в `surface` для трассируемости.
...
def _expand_nmod(
    ...
    Все `nmod`/`nmod:*` (включая `nmod:poss` — притяжательные определения,
    и предложные/генитивные) — это отдельные сущности, связанные ребром.
    """
